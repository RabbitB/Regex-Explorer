...
onready var browse_file_dialog: FileDialog = $FileDialog as FileDialog
onready var address_line_edit: LineEdit = $AddressLineEdit as LineEdit
...
func update_path(new_path: String, update_ui_text: bool) -> void:
	#	Path always matches the value of address_line_edit, so this will effectively auto-update the path.
	if update_ui_text:
		address_line_edit.text = new_path

	if is_path_valid():
		_error = _target_dir.open(self.path)

		if !_error:
			browse_file_dialog.current_dir = self.path
